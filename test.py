...
def main():
    parser = argparse.ArgumentParser(description='Test DeForge-AI on full AIGC-Detection-Benchmark')
    parser.add_argument('--checkpoint', type=str, default='checkpoints/model_epoch_best.pth', help='Path to model checkpoint')
    parser.add_argument('--batch-size', type=int, default=16)
    parser.add_argument('--image-size', type=int, default=256)
    parser.add_argument('--limit', type=int, default=None, help='Limit total number of samples to test')
    ...
    if isinstance(lora_target_modules, str):
        model_kwargs['lora_target_modules'] = [m.strip() for m in lora_target_modules.split(',') if m.strip()]
    ...
    dataset = load_dataset('TheKernel01/AIGC-Detection-Benchmark', split='test', token=hf_token)
    if args.limit:
        dataset = dataset.select(range(min(args.limit, len(dataset))))
        
    test_ds = BenchmarkDataset(dataset, transform=get_val_transforms(size=args.image_size))
    ...
    real_mask = (labels == 0)
    fake_mask = (labels == 1)
    real_acc = (binary_preds[real_mask] == labels[real_mask]).mean() if real_mask.any() else 0
    fake_acc = (binary_preds[fake_mask] == labels[fake_mask]).mean() if fake_mask.any() else 0
    
    print('\n' + '='*40)
    print(f'Overall Results (Full Test Set)')
    print('-'*40)
